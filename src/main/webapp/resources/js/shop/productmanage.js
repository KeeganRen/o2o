...
$(function() {
			...
			if (data.success) {
				...
				productList.map(function(item, index) {
					...
					if (item.enableStatus == 0) {
						// 若状态值为0，表明为已下架的商品，操作变为上架（即点击按钮上架相关商品）
						textOp = '上架';
						contraryStatus = 1;
					} else {
						contraryStatus = 0;
	...
	// 将class为product-wrap里的a标签绑定点击事件
	$('.product-wrap').on('click', 'a', function(e) {
			var target = $(e.currentTarget);
			if (target.hasClass('edit')) {
				// 如果有class edit则点击进入店铺信息编辑页面，并带有productId的参数
				window.location.href = '/o2o/shop/productoperation?productId=' + e.currentTarget.dataset.id;
